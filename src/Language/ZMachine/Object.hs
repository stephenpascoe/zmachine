...
readObjects = do header <- M.getHeader
                 let offset = M.objectTable header
                     propDefaultsLen = if M.zVersionToInt (M.zVersion header) < 4 then 31 else 63
...
readPropDefaults :: M.HasMemory env => M.ByteAddress -> Int -> RIO env PropertyDefaults
...
readObjectRec :: M.HasMemory env => M.ByteAddress -> RIO env (ObjectRec, M.ByteAddress)
...
readObjectRecs offset = f offset Nothing where
  f :: M.HasMemory env => M.ByteAddress -> Maybe M.ByteAddress -> RIO env [ObjectRec]
...
decodePropertyBlock version 
  | (M.zVersionToInt version) < 4 = do  sizeByte <- getWord8
  ...
  | otherwise = do  sizeByte1 <- getWord8
...
decodeObjectRec version
  | (M.zVersionToInt version) < 4 = do attributes <- getByteString 4
                                       parentId' <- getWord8
                                       siblingId' <- getWord8
                                       childId' <- getWord8
  ...
  | otherwise                      = do attributes' <- getByteString 6
